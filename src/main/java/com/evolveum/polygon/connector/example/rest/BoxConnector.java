...
public class BoxConnector
	...
	public Schema schema() {
		if (null == schema) {
			...
			CollaborationHandler collaboration = new CollaborationHandler(configuration);
			ObjectClassInfo collaborationSchemaInfo = collaboration.getCollaborationSchema();
			schemaBuilder.defineObjectClass(collaborationSchemaInfo);
			...
			ObjectClassInfo membershipSchemaInfo = membership.getMembershipSchema();
			schemaBuilder.defineObjectClass(membershipSchemaInfo);
	...
	public Uid update(ObjectClass objectClass, Uid uid, Set<Attribute> attributes, OperationOptions operationOptions) {
		...
		} else if (objectClass.is(COLLABORATION_NAME)) {
			CollaborationHandler collaboration = new CollaborationHandler(configuration);
			return collaboration.createOrUpdateCollaboration(uid, attributes);

		} else if (objectClass.is(MEMBERSHIP_NAME)) {
			MembershipHandler membership = new MembershipHandler(configuration);
			return membership.createOrUpdateMembership(uid, attributes);
	...
	public void executeQuery(ObjectClass objectClass, Filter query, ResultsHandler handler, OperationOptions options) {
		...
		if (query instanceof StartsWithFilter && ((StartsWithFilter) query).getAttribute() instanceof Name) {
			if (objectClass.is(ObjectClass.ACCOUNT_NAME)) {
				...
				FoldersHandler folder = new FoldersHandler(configuration);
				folder.executeCollabQuery(objectClass, query, handler, options);
			} else if (objectClass.is(COLLABORATION_NAME)) {
				CollaborationHandler collaboration = new CollaborationHandler(configuration);
				collaboration.executeCollabQuery(objectClass, query, handler, options);
			} else if (objectClass.is(MEMBERSHIP_NAME)) {
				MembershipHandler membership = new MembershipHandler(configuration);
				membership.executeCollabQuery(objectClass, query, handler, options);
			}
			...
			// EqualsFilter
				...
				// Contains filter made like Startwith filter for Name!
				...
				// ContainsAllValues made like Startwith filter for name !
	...
	public void delete(ObjectClass objectClass, Uid uid, OperationOptions operationOptions) {
		...
		if (objectClass.is(ObjectClass.ACCOUNT_NAME)) {
			...
			CollaborationHandler collaboration = new CollaborationHandler(configuration);
			collaboration.delete(objectClass, uid, operationOptions);
		} else if (objectClass.is(FOLDER_NAME)) {
			FoldersHandler folder = new FoldersHandler(configuration);
			folder.delete(objectClass, uid, operationOptions);
		} else if (objectClass.is(MEMBERSHIP_NAME)) {
			MembershipHandler membership = new MembershipHandler(configuration);
			membership.delete(objectClass, uid, operationOptions);
		}
	}
	...
	public Uid create(ObjectClass objectClass, Set<Attribute> attributes, OperationOptions operationOptions) {
		...
		if (objectClass.is(ObjectClass.ACCOUNT_NAME)) { // _ACCOUNT_
			...
			CollaborationHandler collaboration = new CollaborationHandler(configuration);
			return collaboration.createOrUpdateCollaboration(null, attributes);
			...
			MembershipHandler membership = new MembershipHandler(configuration);
			return membership.createOrUpdateMembership(null, attributes);
